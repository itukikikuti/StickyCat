...
Shader "Hidden/BlurEffectConeTap" {
	...
	half4 frag(v2f i) : SV_Target {
		half4 color = tex2D(_MainTex, i.taps[0] + half2(0, _Offset));
		color += tex2D(_MainTex, i.taps[1] + half2(0, _Offset));
		color += tex2D(_MainTex, i.taps[2] + half2(0, _Offset));
		color += tex2D(_MainTex, i.taps[3] + half2(0, _Offset));
		return color * 0.25;
	}
	ENDCG
	SubShader {
